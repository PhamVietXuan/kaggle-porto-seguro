...
def Multiply_Divide(train, test, features):
    ...
    """
    feature_names = []
    # Tạo tổ hợp chập 2 các feature và đánh số cho từng tổ hợp bằng hàm emurates
    for e, (x, y) in enumerate(combinations(features, 2)):
        # Thực hiện tạo thêm các cột mới là kết quả nhân/chia các feature x, y tương ứng
...
def Statistic_features(train, test, target_features, group_features):
    ...
    for t in target_features:
        for g in group_features:
            if t != g:
                s_train, s_test = proj_num_on_cat(
                    train, test, target_column=t, group_column=g)

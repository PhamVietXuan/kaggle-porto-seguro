...
def cat_count(train_df, test_df, cat_list):
    ...
    train_df['train'] = 1
    test_df['train'] = 0
    all_df = train_df[['row_id', 'train'] +
                      cat_list].append(test_df[['row_id', 'train'] + cat_list])
    for e, cat in enumerate(cat_list):
        ...
        selected_train.drop(['row_id', 'train'] +
                            cat_list, axis=1, inplace=True)
        selected_test.drop(['row_id', 'train'] +
                           cat_list, axis=1, inplace=True)

        selected_train, selected_test = np.array(
            selected_train), np.array(selected_test)
...
def proj_num_on_cat(train_df, test_df, target_column, group_column):
    ...
    :param group_column: name of categorical feature m
    """
    train_df['row_id'] = range(
        train_df.shape[0])  # 595211 create index for each row
    ...
    the_size = pd.DataFrame(grouped.size()).reset_index()
    the_size.columns = [group_column, '%s_size' %
                        target_column]  # rename columns name

    # find the mean, std, median, max, min of each distint value
    the_mean = pd.DataFrame(grouped.mean()).reset_index()
    the_mean.columns = [group_column, '%s_mean' %
                        target_column]  # rename columns name
    ...
    # merge them
    the_stats = pd.concat([the_size, the_mean.iloc[:, 1], the_std.iloc[:, 1], the_median.iloc[:, 1],
                          the_max.iloc[:, 1], the_min.iloc[:, 1]], axis=1, join_axes=[the_size.index])
    ...
    selected_train.drop([target_column, group_column,
                        'row_id', 'train'], axis=1, inplace=True)
    selected_test.drop([target_column, group_column,
                       'row_id', 'train'], axis=1, inplace=True)
...
# Hàm này tạo feature mới có đầu là inter là những đặc trưng được nhân, chia từng cặp feature
